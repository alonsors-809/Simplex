...
void PrintTablaInicial(){
    ...
    Latex_PrintTable(0, -1, -1, false, false);
}
...
void PrintTablaFinal(){
    ...
    Latex_PrintTable(-1,-1, -1, false, false);
}
...
void PrintTablaIntermedia(int columna_escogida){
    ...
    PrintMatriz();
    numero_tabla_intermedia ++;
    Latex_PrintTable(numero_tabla_intermedia, -1, columna_escogida, true, true); // Canonizada
}
...
//        printf("Pivote:%d y pasé por j:%d (%.1f)\n", i_pivote,j, Matriz[i_pivote][j]);
    ...
    Latex_PrintTable(-2,-1, -2, true, true); // Ms canonizadas
//    PrintTablaIntermedia();
}
//
...
//    MatrizDePrueba5();
    ...
    {printf("\n\n\n NO IF TABLAS INTERM \n\n\n");
        Latex_Write("\\section{Intermediates Tables} \n");
    ...
    while (ExisteColumnaCandidata()){
        ...
        puts("");
        if (tablas_intermedias) Latex_PrintTable(numero_tabla_intermedia,  i_pivote,  columna_escogida, false, true); // Tabla intermedia con Pivoteo
